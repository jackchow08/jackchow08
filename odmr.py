...
from tools.file_utility import save_file, open_file
...
class ODMR(ManagedJob, GetSetItemsMixin):
    ...
    frequency_begin_p = Range(low=1, high=3.3e9, value=1.55e9, desc='Start Frequency Pmode[Hz]', label='Begin Pmode[Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    frequency_end_p = Range(low=1, high=3.3e9, value=1.555e9, desc='Stop Frequency Pmode[Hz]', label='End Pmode[Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    randomize_interval = Range(low= 1, high=1000, value= 1, label='Randomize Interval [Runs]', evaluate=int, auto_set=False, enter_set=True)
    ...
    traits_view = View(
        ...
        title='ODMR', width=1000, height=800, buttons=[], resizable=True, x=920, y=0, handler=ODMRHandler
    )
